...
Numerous approaches where considered when planning the implementation of the SMACC software (See \ref{da} for alternate approaches). The approach that was chosen is what I have named the "Sensor Server". The Sensor Server is based on the idea that a GNSS receiver and a computer can be viewed abstractly as a single device, a Sensor. A sensor runs a trivial program that receives data from a GNSS receiver, formats the data correctly and sends the data to the server (more about the client \ref{sensor_client}). With this approach, a great number of GNSS receivers can be connected to the SMACC using existing infrastructure, communicating over IP. 

\subsection{Architecture}

\subsection{Processes vs Threads}\label{procvsthread}
...
Having shared memory segments comes with a price. Whenever two or more processes are working on the same data set, you are prone to race conditions, deadlocks and data corruption. Therefore, semaphores are used to lock the segments while reading or writing are done. 
...
\section{Alternate approaches}\label{da}
