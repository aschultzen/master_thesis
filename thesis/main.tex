...
\subsection{Managing the client list}\label{create_client}
As previously explained, the list of clients are stored in a shared memory segment using a linked list structure. In order to keep tabs on what part of the shared memory segment is in use, a simple map of the memory is used:
\begin{code}
  \caption{Sample of code showing allocation of the \texttt{client\_list\_map}}
    \begin{minted}
    [
    fontsize=\footnotesize,
    fontfamily=tt,
    ...
    \end{minted}
    \label{client_list_map}
\end{code}
The map is an array containing pointers offset by the size of the \texttt{client\_table\_struct}, and maps 1:1 to what can be thought of as the individual \textit{pieces} of memory in the shared memory segment allocated for the client list. A free piece of memory is in the map represented by a pointer that is not NULL. A free piece of memory is returned by the \texttt{get\_mem\_block}. 
\begin{code}
...
\subsubsection{Linked list implementation}\label{linked_list}
