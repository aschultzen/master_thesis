...
\begin{itemize} % Sources needed here?
  ...
  \item Handler for SIGINT (interrupt) SIGTERM (terminate in a controlled manner) and SIGCHLD (child process is interrupted or terminated) are registered.
  \item Initializes socket(\ref{sockets}) and marks it for listening.
  \item Enters loop. The loop breaks when \texttt{volatile sig\_atomic\_t done}\label{atomic_done} equals 0. 
  \item If a client connects, the client table is updated (\ref{create_client}), a fork is performed, the file descriptor to the new clients socket is closed and the loop continues.
  \item If the server loop is broken, the semaphores are destroyed and all allocated memory is freed, the servers socket is closed and the Server exits.
\end{itemize}

\subsection{Parser and Handler}
\begin{figure}
\centering
  \includegraphics[angle=90, scale=0.3]{server_flow.pdf}
   ...
   \label{server_flow}
\end{figure}
The parser and handler are responsible for parsing data received from a Client and making sure every request is protocol compliant. Once the goal of the request has been established, the request is executed. 
...
\subsubsection{config\_loader()}\label{config_loader}
Write about the config loader here

\subsubsection{nmea\_ready()}\label{nmea_ready}
Write about nmea\_ready() here.

\subsubsection{\texttt{create\_client()}}\label{create_client}
\texttt{create\_client()} takes a pointer to the client list as parameter, locks the \texttt{client\_list\_m}
...
\subsubsection{Linked list}\label{linked_list}
...
\begin{code}
  ...
  \label{server_data}
\end{code}
The \texttt{server\_data} structs members are 
