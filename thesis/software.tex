...
\begin{itemize} % Sources needed here?
  ...
  \item Shared memory segments and semaphores are initialized and allocated. See section \ref{shared_mem_sem} for more. If the allocation fails, the Server prints an error message and exits.
...
\end{figure}
Conceptually, the CSAC model and the filter that uses the model is two separate tasks. In practice however, they are intertwined as figure \ref{figure_csac_model} suggests.
...
\begin{itemize}
  ...
  \item A check is done to see if the model has been running for at least 48 hours. If it has, it is ready to be used a reference for the filters. See section \ref{csac_filters} for more about the CSAC filters.
  \item The telemetry received from the CSAC is used to update the model. 
  \item A check is done to see how if it is time to calculate the daily prediction values. Every 24 hours, the predictions are calculated. The models keeps track of the prediction for yesterday and for today. By using these two points of data, an estimation of the predicted steer can be calculated. 
\end{itemize}
...
\begin{code}
  \caption{Example of a telemetry string received from the CSAC}
    \begin{minted}
    [
      fontsize=\footnotesize,
      fontfamily=tt,
