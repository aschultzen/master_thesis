...
When discussing how to implement the software realizing the spoof proof atomic clock controller, the following features and requirements were identified: 
\begin{itemize}
...
\end{figure}
Conceptually, the atomic clock model and the filter that uses the model is two separate tasks. In practice however, they are intertwined as figure \ref{figure_csac_model} suggests. See section \ref{hw_csac} for more about the atomic clock and section \ref{clock_model_harald} for the model itself.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textit{Timing jump filter}. This is a simple filter that compares the current phase reported by the atomic clock with a pre-configured limit. If the current phase is higher, the filter is triggered.
