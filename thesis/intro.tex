...
\subsection{Global Positioning System: A short introduction}
...
\subsection{GPS signals and Time}\label{GST}
...
\paragraph{Signal-level Spoofing}\label{sls}
...
\paragraph{Data-level Spoofing}\label{dls}
...
\paragraph{Replay spoofing}\label{rs}
...
\paragraph{Malfunctions}\label{mf}
...
\paragraph{Monitoring Signal Power}\label{msp} %[C1]
...
\paragraph{Checking solved position against known position}\label{cspakp} %[C7]
...
\paragraph{Checking time solutions against receiver clock statistics}\label{ctsarcs} %[C8]
...
\paragraph{Cross-checking navigation data among receivers}\label{ccndar} %[C5]
...
\paragraph{Comparing navigation data and reverse-calculated satellite positions}\label{cndrcsp} %[C6]
...
\paragraph{Cross-correlating P(Y) code}\label{ccpc} %[C2]
...
\paragraph{Position Aided (PIA) Tracking loops}\label{ptl} %[C3]
...
\paragraph{Multi-receiver tracking loops}\label{mrtl}
...
\paragraph{Spoof proof CSAC SMACC}\label{op}
\begin{wrapfigure}{c}{0.45\textwidth}
  \centering
...
\iffalse  
We propose to use a smart miniature atomic clock controller(SMACC) with a chip scale atomic clock(CSAC) to complement a commercial GPS controlled clock to harden it against attacks. The Symmetricom SA.45 is CSAC measuring only 16cc with 1 pulse per second (PPS) output and 1 PPS input (for sync) as well as a RS-232 interface. The key element in our proposal is that we have a clock that we can trust and use as a reference. Any correction needed as implied by the GPS signal will be checked with the CSAC.
If the correction is to big, an alarm will be raised. In the event of a jamming attack (or malfunction in the receiver), the CSAC can be used to correct a GPS controlled clock instead of the 1 PPS output from the GPS receiver. In the event of a spoofing attack (\ref{sls},\ref{dls},\ref{rs}) on a PMU with a GPS controlled clock, the stable nature of the CSAC can be used by a controller to detect the attack and to disable the GPS correction and instead rely on the CSAC for correction. When used with an atomic clock, the controller can simply disable the GPS correction since the atomic clock would keep better time than the CSAC.
...
It is also important to note that this approach doesn't really do anything with the fact that you are being attacked, it is simply an attempt at eliminating the effects of it. In a scenario where you are under attack weeks at a time, you will have to address the issue that you are under attack at some point. It is also important to notes that this countermeasure mostly apply to applications using GPS as a source of time. Having a stable clock during a jamming attack will not help you determine your position once you move (given that you are fully jammed).
