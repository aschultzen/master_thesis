...
The goal with this test was to use the atomic clock controller to detect a simulated spoofing attack. By moving the antennas, the LS filter should be triggered as the solved longitude, altitude, latitude and speed should change. See \ref{kvsrlf} for more about the LS filter. The result would be observable by analyzing the log files produced by the Server and Clients and the data collected by the measuring setup.
