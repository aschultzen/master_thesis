...
We propose to use a smart miniature atomic clock controller(SMACC) that controls a CSAC (chip scale atomic clock). The Symmetricom SA.45 is CSAC measuring only 16cc with 1 pulse per second (PPS) output and 1 PPS input (for disciplining) as well as a RS-232 interface. Playing the role as the SMACC will be a Raspberry Pi with software that implements the following countermeasures:
...
\begin{figure}[h!]\label{bd}
  \includegraphics[width=1\textwidth]{master_sketch_arrows.pdf}
