...
\definecolor{effectivec}{RGB}{70,240,161}
\definecolor{auxiliryc}{RGB}{115,70,240}
\definecolor{ineffectivec}{RGB}{70,149,240}
...
\end{wrapfigure} 
We propose to use a smart miniature atomic clock controller(SMACC) with a chip scale atomic clock(CSAC) to complement a commercial GPS controlled clock to harden it against attacks. The Symmetricom SA.45 is CSAC measuring only 16cc with 1 PPS output and 1 PPS input (for sync) as well as a RS-232 interface. The key to our proposal is that we have a clock that we can trust and use as a reference. Any correction needed as implied by the GPS signal will be checked with the CSAC. If the correction is to big, an alarm will be raised. In the event of a jamming attack, a CSAC like this can be used to correct a GPS controlled clock instead of the 1 PPS output from a GPS receiver. In the event of a spoofing attack on an PMU with a GPS controlled clock, the stable nature of the CSAC can be used by a controller to detect the attack and to disable the GPS correction and instead rely on the CSAC for correction. When used with an atomic clock, the controller can simply turn of the GPS correction since the atomic clock would keep better time than the CSAC. It's the same story with replay attacks, once again having a trusted reference clock enables us to detect the attack.
...
Some text here, whatever.
  ...
  \begin{table}[h]
    ...
    \hline
    \cellcolor{effectivec}Y & Effective & \cellcolor{ineffectivec}N & Ineffective & \cellcolor{auxiliryc}X & Auxiliary \\ \hline
    \end{tabular}
